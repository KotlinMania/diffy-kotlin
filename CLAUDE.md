# Claude Code Project Instructions — diffy-kotlin

## Project Overview

This is **diffy-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`diffy`](https://crates.io/crates/diffy).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/diffy/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/diffy>
